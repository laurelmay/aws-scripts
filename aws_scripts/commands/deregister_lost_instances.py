...
_CONNECTION_LOST_FILTERS = [
    {"key": "PingStatus", "valueSet": ["ConnectionLost"]},
    {"key": "ResourceType", "valueSet": ["ManagedInstance"]},
]
...
def main(profile: str, page_size: int) -> None:
    ...
    for page in paginator.paginate(
        ...
        for instance in page_instances:
            ...
            if ping_date:
                ping = datetime.strftime(
                    instance["LastPingDateTime"], "%Y-%m-%d %H:%M:%S"
                )
            else:
                ping = "Never"
    ...
    final_confirm = click.confirm(
        "Are you sure you want to deregister all the above instances?"
    )
    ...
    with click.progressbar(
        lost_instances, item_show_func=get_instance_name
    ) as instance_bar:
